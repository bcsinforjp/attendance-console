...
2026-06-10 — [app-update] **Fixed `/download/desktop` 404 ("No desktop app build available").** `app_update.json` still pointed at `GenbaLink-Setup-2.4.26.exe` while the file on disk (uploaded today) was `GenbaLink-Setup-2.4.35.exe`; the route looked up the stale filename and 404'd. Repointed the config to the real file with recomputed `sha256=0b947fe7…186c45`, `size=269857118`, `version=2026.06.10-0938` (backup at `app_update.json.bak`). Same config drives desktop self-update, so 2.4.35 is now the offered build (`mandatory:false`). Note: the installer is **unsigned** (empty PE cert table) → browsers/SmartScreen warn "unknown publisher"; the fix is a code-signing cert, not a code change. → `app_update.json`.

2026-06-10 — [management] **Added the "接続・連携 / Connect & Integrate" card to `/management`.** Same card as the home page (⬇ Desktop App `/download/desktop`, 🤖 AI Integration Guide `/guides`, 📖 App guide `/desktop-guide`), placed at the top above the password unlock so it's reachable regardless of lock state. → `static/management.html`.

2026-06-10 — [agent-api] **`get_attendance_summary` per-employee report endpoint — agent change request #6.** `GET /api/agent/v1/attendance-summary?start=&end=&section=&format=json|csv` + MCP tool `get_attendance_summary` (now 18 tools). Per employee over a range (≤92 days): `{code, name, section_id, present_days, absent_days, total_minutes, total_hours_hhmm, overtime_minutes/overtime_hhmm (>8h/day), attendance_rate_pct}` + response totals; CSV download. Built by aggregating `_attendance_export_rows` over the indexed `attendance_records` (`_attendance_summary_rows`) — no new tables. Covers #6's core (days worked, hours, overtime, attendance rate, absences, range/section filter, CSV). #6 evaluated→approved→in_progress; server-side Excel/PDF + productivity ranking noted as a future request (the agent can format JSON/CSV into Excel/PDF itself now). Verified: Jun 1–10 → 101 employees. → `agent_api.py`.

2026-06-10 — [home] **Home page (`index.html`) now links the Desktop App download + AI guide.** Added a "接続・連携 / Connect & Integrate" card after the hero with buttons: ⬇ Desktop App (`/download/desktop`), 🤖 AI Integration Guide (`/guides`), 📖 App guide (`/desktop-guide`); plus "Desktop App" + "AI Guide" entries in the top nav. Root `/` route now sends `no-store` so edits show on reload. → `index.html`, `main.py`.

2026-06-10 — [agent-api/oauth] **Separate "connector password" for MCP OAuth approval + admin UI to manage it.** ChatGPT/Claude can now add the server as an **MCP connector** (URL `…/mcp`, OAuth) — the `/oauth/authorize` approval page accepts a **dedicated `mcp_connect_password`** (in `admin_config.json`) in addition to the admin console password, so connector access can be shared without exposing the admin password (`mcp_oauth._connect_password()`; either password approves). Added a **"MCP Connector" card** to Admin → AI Agents showing the connector URL + a field to view/set the connector password (`GET/POST /admin/api/agentapi-connect-pw`). Approval page wording → "connector password". Default `GenbaConnect2026` (changeable in the UI). → `mcp_oauth.py`, `main.py`, `static/admin.html`, `admin_config.json`.

2026-06-10 — [reports] **Back button on report views.** In `?report=1` mode (the clean print view of `/gantt` and `/summary` opened from Reports), `site_header.js` now injects a fixed top-left **← レポートに戻る / Back to Reports** link (→ `/reports`, mount-aware), hidden via `@media print`. Pairs with the earlier change that opens reports in the same window. Bumped `site_header.js?v=` → 20260610-back on all 7 pages. → `static/site_header.js`.

2026-06-10 — [nav] **Unified the top navigation across every page to match the HMI tab set.** Rewrote the shared `site_header.js` nav from the old 5 links (Status/Reports/Setup/Intake/Lens) to the **same 7 tabs as the HMI**, same order/labels/icons: 📺 監視概要 (/dashboard) · 📊 ガント (/gantt) · 📈 集計 (/summary) · 🔍 メンバー時間 (/lens) · 📄 レポート (/reports) · 🖥️ コンソール (/console) · 👥 名簿管理 (/management). Updated `activeKey()` to one key per page and restyled the active tab to solid teal (#0d9488, white text) to match the HMI's current-tab look. So all operator pages now show the identical tab bar with the current page highlighted, consistent with /dashboard. **Follow-up:** also brought the **HMI-style top** to every page via `site_header.js` — added a live **KPI cluster** (本日パック / Packs from `/api/dashboard/snapshot`, 稼働ライン / Lines running, refreshed 60s), a **JP/EN language toggle** (scoped to the topbar so page content is untouched; persisted in `hmiLang`, makes the now-bilingual nav labels switch), and a **⛶ fullscreen button** — alongside the existing clock + health pill. Then matched the HMI top more closely: brand renamed **現場 FMS → GENBA FMS** with a per-page bilingual subtitle (e.g. "GENBA FMS · ガント"), and added a **3rd KPI (日付 / Date)** from `report_date`. The HMI-specific "DEMO SENSORS · LIVE KPI" badge and "Line Overview" subtitle were intentionally NOT propagated (only the HMI shows simulated sensors). Bumped `site_header.js?v=` (→ 20260610-top2) on all 7 pages. **Then split the header into two cards to mirror the HMI** (`.top` + `.tabs`): the 7 nav tabs moved out of the topbar into a **separate `.site-tabs` card directly below it** (white card, teal active tab, scrolls on mobile); the health/agent/user pills stay in the top card's middle. Mobile grid simplified to a single column. Bumped `?v=` → 20260610-2card. → `static/site_header.js`, `?v` bump on 7 pages.

2026-06-10 — [theme] **HMI converted to the app's LIGHT theme; site-wide dark experiment reverted.** Briefly trialled a shared dark override (`genba-theme.css` injected via `site_header.js`) across all pages, then **rolled it back per request** — the inject block was removed from `site_header.js` and `genba-theme.css` deleted, so the other pages (gantt/lens/summary/reports/console/management) keep their original light theme. The **HMI/dashboard overview was re-skinned to light** to match: white panels (#fff) on a soft #f6f8fa background, GenbaLink teal accent (#0d9488/#0b6f58), dark ink text (#1f2933), light readout/worker chips, light conveyor track with teal flowing dots, gauge track #e3e8ee, colored zone bars + green/amber/red status preserved. Kept the JP/EN toggle, top tab bar, fullscreen, graphics, and live-jitter. Bumped `site_header.js?v=` 20260610-theme → 20260610-light on all 7 pages so the revert propagates. Also (kept) the earlier HMI fixes: contained status-lamp glow (no overlap) + ▶ FEED / ■ STOP conveyor. Backup: `static/site_header.js.bak.20260610`. → `static/hmi.html`, `static/site_header.js`, removed `static/genba-theme.css`, `?v` bump on 7 pages.

2026-06-10 — [docs] **Guide JP/EN toggle + per-user onboarding card.** (1) Rewrote both guides (`AI_INTEGRATION_GUIDE.md`, `DESKTOP_APP_GUIDE.md`) with inline `<j>…</j><e>…</e>` language tags; added a **日本語/EN toggle (default JP)** to all three viewers (`guides.html`, `ai_guide.html`, `desktop_guide.html`) via `body.lang-jp e{display:none}` etc. — one language shown at a time. (2) New **`/onboard`** per-user card (`static/onboard.html`): pre-fills a person's **API key + GenbaLink login + desktop app password** with copy buttons + setup steps for both the desktop app and ChatGPT. Credentials are read from the URL **`#hash`** (client-side only — never sent to the server, so they're not written to access logs), with a clear "keep this link private" warning + password reveal toggle. Public routes added to `GBL_PUBLIC_PREFIXES`. → `static/onboard.html` (NEW), guide MDs + 3 viewers, `main.py`.

2026-06-10 — [docs] **Guides hub at `/guides` — switch between AI & Desktop guides in one page.** New `static/guides.html`: two icon cards (🤖 AI/ChatGPT with a teal robot SVG, 💻 Desktop App with an indigo monitor SVG); clicking a card renders that guide's markdown inline (fetches `/ai-guide.md` or `/desktop-guide.md`, marked.js), with active-card highlight, `#ai`/`#desktop` deep-link hash, and last-choice persistence. Public route `/guides`. The standalone `/ai-guide` and `/desktop-guide` still work. → `static/guides.html` (NEW), `main.py`.

2026-06-10 — [docs] **Public Desktop App user guide at `/desktop-guide`.** New `DESKTOP_APP_GUIDE.md` — operator-facing (JP-first/EN) guide for the 現場Link/GenbaLink Windows app: what it does, install (download link + SmartScreen "Run anyway"), first-time setup (app login, Settings: API key/GenbaLink login/watch folders/printer/language), daily use (status bar, Sync Now), print/save reports, scheduled LINE delivery (with the FMS LINE QR @229ttzgw + "send hi" registration), and troubleshooting. Rendered at **`/desktop-guide`** (`static/desktop_guide.html`, reuses the ai_guide viewer) + raw `/desktop-guide.md`; both public. No secrets baked in (app password / API key come from the admin). → `DESKTOP_APP_GUIDE.md` (NEW), `static/desktop_guide.html` (NEW), `main.py`.

2026-06-10 — [docs] **Personalized ChatGPT setup card at `/chatgpt-setup?key=…`.** New `static/chatgpt_setup.html` — a one-page, copy-button setup for a single person: reads the agent key from the `?key=` URL param (NOT baked in, so the public `/ai-guide` never carries a key), pre-fills it + the OpenAPI schema URL, and walks the 5 Custom-GPT-Action steps. Public route `/chatgpt-setup` (no-store). Admin sends one person a link like `…/chatgpt-setup?key=at_…`; key stays valid until revoked in Admin → AI Agents. → `static/chatgpt_setup.html` (NEW), `main.py`.

2026-06-10 — [docs/download] **AI guide reworked to ChatGPT-only + 5-subsystem breakdown + per-subsystem links + public desktop-app download.** Rewrote `AI_INTEGRATION_GUIDE.md`: removed all Claude/MCP content (ChatGPT-only, with step-by-step "screenshot-style" Custom-GPT-Action setup, QR, and a paste-ready brief); simplified the server description to just "Linux server" (dropped Raspberry Pi / Cloudflare wording); restructured "big picture" into **5 clearly-listed subsystems** (① Linux Server ② Desktop App ③ Website-any-device ④ ChatGPT ⑤ Admin/Buddhika), each with its features and a working **🔗 link**. Added a **public desktop-app installer download** — new `GET /download/desktop` serves the currently-published installer from `app_update.json` (`GenbaLink-Setup-2.4.26.exe`, attachment disposition), added to `GBL_PUBLIC_PREFIXES`. → `AI_INTEGRATION_GUIDE.md`, `main.py`.

2026-06-10 — [docs] **Public AI Integration Guide at `/ai-guide` (+ QR).** New `AI_INTEGRATION_GUIDE.md` — bilingual (EN/JP) end-to-end guide: the big-picture architecture (Linux server + cross-platform desktop app + AI agents, and *why* each exists), what an AI agent can do (data / calculations / reports / chat), the **project-request workflow** (anyone asks ChatGPT in any language → server → Buddhika reviews via admin/AI → status tracked; reduces the language barrier), easy ChatGPT/Claude connection methods (Custom GPT Action via the `openapi.json`, paste-brief, Claude connector), a quick tool reference, and the safety model. Served rendered at **`/ai-guide`** (new `static/ai_guide.html`, marked.js) and raw at **`/ai-guide.md`**; both public (added to `GBL_PUBLIC_PREFIXES`). The guide embeds a **QR** (via api.qrserver.com) that opens the guide on a phone. → `AI_INTEGRATION_GUIDE.md` (NEW), `static/ai_guide.html` (NEW), `main.py`.

2026-06-10 — [agent-api/perf] **Fast data reads: 8s in-memory cache on the agent read-proxy + composite index.** `_proxy_json` in `agent_api.py` now caches parsed responses for 8s (keyed by path; size-bounded at 256), so repeated polls (HMI every 15s, dashboards, agents re-reading the same date) skip the loopback hop + DB query — measured **~35× faster on warm hits** (`/production` 103ms→3ms, `/attendance-range` 56ms→3ms). `_proxy_write` calls `_cache_clear()` so any write is reflected immediately (max staleness = 8s, writes = 0s). Added composite index `idx_attendance_code_date ON attendance_records(personal_code, record_date)` for the per-employee range endpoint. (DB tables are small + already indexed, so the cache — not indexing — is the main win.) → `agent_api.py`, Postgres index.

2026-06-10 — [agent-api/perf] **Bulk attendance export + `X-Response-Time` — agent change request #5 (P1 + measurement).** (1) **`GET /api/agent/v1/attendance-export?start=&end=&section=&format=json|csv`** + MCP tool `get_attendance_export` — ALL employees' daily `{code,name,section_id,date,minutes,in,out,anomaly}` over a range (≤92 days) in **one call**, JSON or **CSV download**; built on the indexed `attendance_records` via batched `/api/members/compare` (30 codes/batch, `_attendance_export_rows`). (2) Every agent response now carries an **`X-Response-Time: <ms>`** header (in `_envelope` + the CSV path) so clients can measure latency. (3) Confirmed `get_attendance_range` + the new export are in MCP `tools/list` (17 tools) — P1-1 was a client-side tool-cache refresh, not a server gap. Verified: 3-day export = 303 rows, CSV OK, >92d → 400, header present (119ms). #5 evaluated→approved→completed with a reply; P2/P3-2/P4 noted as deferred (export already covers annual; Cloudflare handles wire compression; reads already 8s-cached). → `agent_api.py`.

2026-06-10 — [agent-api] **New `get_attendance_range` endpoint/tool — agent change request #4 (fast per-employee range retrieval).** `GET /api/agent/v1/attendance-range?employee_code=&start=&end=&section=` + MCP tool `get_attendance_range`. Returns ONE employee's per-day `{date, minutes, in, out, anomaly}` + `total_minutes`/`total_hours_hhmm`/`present_days` in a **single call** (replaces N day-by-day calls). Implemented in `agent_api.py` by reusing the existing **indexed `attendance_records`** via the multi-member `/api/members/compare` path (`_attendance_range_data`) — deliberately **no new `attendance_daily` table or FTP ingest job** as the request proposed, since the perf goal (1 query, minimal payload, indexed by code+date) is already met; anomaly = `missing_in`/`missing_out` derived from punch presence. Verified (emp 00000376, 2026-06-01..10 → 10 days/6 present/47:54 in one call). Change request #4 evaluated (feasible, low risk) → approved → completed, with a reply to the agent. → `agent_api.py`.

2026-06-10 — [hmi/scada] **HMI conveyor → flowing arrows + 5 line states.** Replaced the conveyor's flowing dots with animated **chevron arrows** (CSS `mask` of a 2-chevron SVG, colored per state via a `--belt` var, `mask-position` animation; paused+dimmed when stopped). Expanded line state from RUNNING/IDLE/FAULT to **5 states** — 稼働中/RUNNING (green, flowing), 待機/IDLE (amber), 停止/STOPPED (gray), 整備中/MAINT. (blue), ライン異常/FAULT (red) — each driving the state badge color/LED, the belt color + flow, and the per-section status badges. The conveyor end-label shows the live state (▶ RUNNING / ■ STOPPED…). → `static/hmi.html`.

2026-06-10 — [hmi/scada] **New `/hmi` SCADA-style all-lines overview (wall display) — Phase-1 of agent change-request #3.** Two-line monitoring screen (Line A = 製造１課, Line B = 製造２課) in GENBA brand colors (Genba Indigo #1A2942 / Signal Teal #00B8A9). **Live/real data** bound from existing public endpoints: per-line packs + target vs actual + progress (`/api/dashboard/snapshot` → today.n_total/y_total, plan.total_planned), rate個/h (productivity `lp`), headcount (`staff_present`), and **real worker names** (`/api/gantt/{latest}` rows). **Simulated demo data (clearly labelled, randomised each 6s refresh):** 12 process-station status lamps + readouts (boil/shrink temperature, checkweigher grams), OEE, reject counts, product/LOT, and per-section RUNNING/IDLE/ERROR badges — these need PLC/sensor feeds the system doesn't collect yet. Worker boxes show real names at full opacity; sparse sections are padded with dimmed `·demo` names so the wall display looks populated. Self-contained `static/hmi.html` (inline CSS/JS like gantt.html/lens.html), auto-refresh 6s, responsive. Served at **`/hmi`** (new route, `no-store`) and added to `GBL_PUBLIC_PREFIXES` (wall-display; the underlying data endpoints are already public). **`/dashboard` now also serves the HMI** — it was a placeholder ("under development"); its route was repointed to `hmi.html` and `/dashboard` added to the public allowlist, so `https://genbafms.com/dashboard` shows the overview bilingually. Old placeholder preserved at `static/dashboard.html` (route reversible). **Same-day add:** a **JP / EN / 両方(both)** language toggle (segmented control, persisted to `localStorage`; all labels are `.jp`/`.en` spans toggled via a body class, with a `/` separator only in both-mode) and a **navigation menu** (☰ drawer) linking the other pages — Overview, Gantt, Summary, Lens, Reports, Console, Roster mgmt, Update log, Admin, and Change requests — grouped under Floor-views / Operations headings. **Iteration 2:** dropped the "両方/both" language option (now **JP / EN only**, default JP); added a **⛶ fullscreen/kiosk button** (`requestFullscreen`); polished the **menu** into an animated popup with a title, section headers, emoji icons, and current-page highlight; added **HMI graphics** — circular **OEE gauge** (SVG arc, green/amber/red by value), animated **conveyor belts** per line (paused/amber when not running), **pulsing status lamps**, blinking state LED, glowing progress bar, brand logo + a **reject-count** tile; and split refresh into **live data every 15s** + **demo-value jitter every 2.5s** (sensor readouts/lamps/OEE/rate re-randomise with a flash animation and thousands-formatted numbers, so the board looks live). Change request #3 evaluated (feasible, risk med) → approved → in_progress, with a thread note linking the live demo. → `static/hmi.html` (NEW), `main.py` (`/hmi` route + allowlist).

2026-06-09 — [agent-api/mcp/oauth] **Added an OAuth 2.1 sign-in server so the MCP endpoint works as a one-click Claude/ChatGPT "custom connector" (no pasted token).** Newer Claude Desktop builds (cowork) ignore `claude_desktop_config.json` and connect remote MCP servers only via in-app Connectors, which require OAuth DCR — connecting failed with *"Couldn't register with GENBA FMS's sign-in service."* Implemented the MCP authorization spec:
  - **`mcp_oauth.py` (NEW)** — minimal OAuth 2.1 AS: RFC 9728 protected-resource metadata (`/.well-known/oauth-protected-resource`), RFC 8414 AS metadata (`/.well-known/oauth-authorization-server` + openid-configuration alias), RFC 7591 dynamic client registration (`POST /oauth/register`), authorization-code + **PKCE S256** (`GET/POST /oauth/authorize`, `POST /oauth/token`), refresh-token grant. Approval is gated by the **admin password** (reuses `admin_config.json`); the authorize page is a small dark-theme form. Clients/tokens persisted to `mcp_oauth.json` (0600). A successful sign-in mints a 30-day access token (`mat_…`) bound to a **managed agent** auto-created per OAuth client (`a_oauth_<client>`, write-enabled), so each connection appears in /admin → Agent API and is revocable/rate-limited like any other agent. Public client (`token_endpoint_auth_methods=["none"]`); base URL derived from `x-forwarded-host` (https for non-loopback).
  - **agent_api.py** — `set_oauth_resolver()` hook + `ensure_oauth_agent()` (get-or-create managed agent, auto-enables the API on first admin-approved connect). `_require_agent` now resolves OAuth `mat_…` tokens (via the injected resolver → agent id) in addition to static `at_…` tokens, so both REST and MCP accept them. `/mcp` now returns **401 + `WWW-Authenticate: Bearer resource_metadata="…/.well-known/oauth-protected-resource"`** when unauthenticated (POST and GET), triggering the client's OAuth discovery; discovery 401 fires even when the API is disabled so the flow can start.
  - **main.py** — `include_router(mcp_oauth.router)`, `set_oauth_resolver(resolve_token)`; added `/.well-known/oauth`, `/.well-known/openid`, `/oauth/` to `GBL_PUBLIC_PREFIXES`.
  - **Verified end-to-end (public host)**: unauth `/mcp` → 401+WWW-Authenticate; protected-resource + AS metadata; DCR → client_id; authorize with admin password → 302 + code; PKCE token exchange → `mat_` access token; OAuth token on `/mcp` → 15 tools + successful tool call; **negatives**: wrong admin password → 401, PKCE mismatch → invalid_grant. OAuth connections show as revocable agents in the admin panel. Test artifacts cleaned; API reset to deny-by-default (a real connect re-enables on admin approval). → `mcp_oauth.py` (NEW), `agent_api.py`, `main.py`.

2026-06-09 — [agent-api/mcp/change-mgmt] **New unified AI-Agent API (REST + MCP) on one bearer token, plus an agent Change-Management workflow.** Two new modules + admin panels let any external AI agent connect with a key to (A) read/write factory data and (B) submit feature/system requests that an admin evaluates → approves → a human implements → the agent reports back. Built on the proven `partner_api.py` pattern (deny-by-default, per-token revocable + rate-limited + IP-allowlistable).
  - **`agent_api.py` (NEW)** — `APIRouter(/api/agent/v1)` + a JSON-RPC MCP handler. Token store `agent_api.json` (0600) `{enabled, agents:[{id:"a_…", name, token:"at_…", rate_per_min, ip_allowlist, disabled, write_enabled, created_at}]}`. `_require_agent` (Bearer/`X-Agent-Token`, `secrets.compare_digest`, disabled/IP/rate-limit, deny-by-default). Reads proxy GET over loopback (`_proxy_json`): `ping, datasets, production, productivity, packs, attendance, roster, dayoff, packs/{date}/history`. Writes proxy POST over loopback (`_proxy_write`, `requests`) — **pack counts + temp-staff only**, gated on per-token `write_enabled` (else `403 write_disabled`): `POST /packs`, `/packs/bulk`, `/temp-staff` → existing `/api/daily-packs`, `/api/daily-packs/bulk`, `/api/temp-staff`. Loopback (Host=127.0.0.1) bypasses `genbalink_host_auth`, so the session-only internal write handlers are reachable server-side while the external caller is gated solely by the agent token (contract documented in `_proxy_write`; upstream 400/409/423 → `bad_request`/`duplicate`/`ingestion_paused`). Enveloped responses with ETag/304. Sync `def` write handlers (FastAPI threadpool) so the blocking proxy never stalls the loop.
  - **MCP** — `POST /mcp` JSON-RPC 2.0 (plain `application/json`, no SSE → no nginx change). `initialize`/`tools/list`/`tools/call`, same `_require_agent` bearer auth (`-32001` on fail), tool work offloaded via `run_in_threadpool`. **15 tools** = 7 read + 3 data-write (write-gated) + 5 change-request. Registered as explicit routes `app.add_api_route("/mcp", …)` (not `app.mount`, which 307-redirected `/mcp`→`/mcp/`). `GET /mcp` → 405.
  - **`change_requests.py` (NEW)** — Postgres ledger (own connection, no circular import). Two tables created idempotently (`ensure_tables()`, called after `init_db()`): `agent_change_requests` (status/feasible/risk_level/timeline_estimate/eval+decision fields) and `agent_change_messages` (author/kind/body/meta jsonb thread). Lifecycle `submitted→under_review→approved|rejected|needs_info→in_progress→completed|failed`. Helpers: create/list/get (with ownership filter)/add_message/add_feedback (advances in_progress→completed|failed)/evaluate/decide/set_status/status_counts.
  - **main.py** — wired `agent_router` + error handler + `/mcp` routes + `ensure_tables()`; added `/api/agent/v1` and `/mcp` to `GBL_PUBLIC_PREFIXES`. Admin endpoints (each `_require_admin_session`): agent-token CRUD as **`/admin/api/agentapi-*`** (status/log/enable/add/update/revoke — `agentapi-` prefix to avoid colliding with the **pre-existing** desktop-agent `/admin/api/agent-status|agent-log`); change-mgmt `/admin/api/change-requests[/{id}/evaluate|decision|status|messages]`; `/admin/agentapi-spec` serves `AGENT_API.md`.
  - **static/admin.html** — two new tabs: **Agent API** (enable/disable, add agent with write toggle + rate + IP allowlist, token shown once, per-token write/disable/revoke, request log) and **Change Requests** (list + status/commitments filter, unread `submitted` badge, detail view with conversation thread, evaluation form, Approve/Reject/Needs-info + In-progress/Completed/Failed controls, reply box). Mirrors the Partner-API panel pattern.
  - **AGENT_API.md (NEW)** — external integration manual (auth, envelope/errors, read/write endpoints, MCP + Claude-client connection, change-mgmt lifecycle, limits).
  - **Verified** (loopback): ping/auth (200/401), read + ETag→304, write + `daily_packs_history` audit-row proof, write-disabled→403, MCP initialize/tools-list(15)/tools-call read+write, change-request full lifecycle (submit→evaluate→approve→in_progress→feedback completed) + cross-agent 404 isolation, and every admin panel endpoint. Test data + tokens cleaned up afterward. Backups: `main.py.bak`. → `agent_api.py` (NEW), `change_requests.py` (NEW), `AGENT_API.md` (NEW), `main.py`, `static/admin.html`.

2026-06-09 — [auth/desktop-agent] **New `/auto-login` token bridge — agent "Print / PDF" buttons now open `/pdf/*` in the user's default browser with no login form.** Request from agent dev (`SPEC_REQUEST_app-auto-login.md`, 2026-05-28): `shell.openExternal('/pdf/gantt?date=…')` lands the user on the GenbaLink login wall because the default browser has no `gbl_session` cookie. Implemented the **token variant** (their §6), **not** the raw-`?api_key=` form they first proposed — putting the long-lived, high-privilege ingestion key in a URL would write it into browser history, the Nginx access log, `agent_requests.jsonl`, and Referer headers. Two endpoints:
  - **`POST /api/v1/auth/login-token`** (`require_api_key`, key in header only) → mints a single-use, 60s token (`secrets.token_urlsafe(32)`) held in a new in-memory store `_login_tokens` + `_login_tokens_lock` (mirrors `_gbl_sessions`); `_login_token_create` / `_login_token_consume` (consume = `dict.pop`, so reuse fails). Returns `{token, expires_in:60, redirect_to:"/auto-login"}`.
  - **`GET /auto-login?token=&redirect=`** (public) → consumes the token, mints a normal `gbl_session` via `_gbl_session_create(username=<api-key label>, is_admin=False)` (service identity — we have no api-key→human-user mapping), sets the same cookie shape as `/api/auth/login` (`HttpOnly; SameSite=Lax; Max-Age=43200`; **no** `Secure` — Nginx adds it on the HTTPS hop), and `302`s to `redirect`. Open-redirect hardened: must be a relative path (no `://`, no `//`, no `/auto-login` loop) **and** match `AUTO_LOGIN_REDIRECT_PREFIXES = ('/pdf/', '/dashboard')`; anything else → 400. Bad/expired/reused token → 401. Each hit logged via `_record_agent("/auto-login", …)` with the token + redirect (never the key) and `X-Agent-Version` / `CF-Connecting-IP`.
  - **`GBL_PUBLIC_PREFIXES`**: added `/auto-login` so the host-auth middleware doesn't bounce it to `/login` before the handler runs (the route self-guards via the one-time token). `POST /api/v1/auth/login-token` is already covered by the `/api/v1` self-protected family.
  - **Verified** (loopback, `attendance.service` restarted): login-token w/o key → 401; w/ header key → 43-char token; `/auto-login` happy path → 302 + cookie; token reuse → 401; `redirect=https://evil…` → 400; `redirect=/admin/secrets` → 400; garbage token → 401; issued cookie → `/api/auth/whoami` `authenticated:true`. Reply doc to agent dev: `SPEC_REPLY_auto-login.md`. Agent side still to wire (2 calls instead of 1). Backup: `main.py.bak`. → `main.py` (new token store + 2 endpoints + allowlist), `SPEC_REPLY_auto-login.md` (NEW).

2026-05-31 — [lens] **Print / PDF report for the Lens result.** New `🖨 Print / PDF` button prints whichever view is active (Daily strip / Weekly grid / Monthly grid) as a clean report — `@media print` hides the site header, controls, chips and empty prompt, drops card borders, expands the monthly grid (overflow visible, sticky name col → static), and sets `@page A4 portrait`. A print-only header (`#memPrintHead`) shows the range, view, and selected members. Browser "Save as PDF" produces the PDF (no server-side render, consistent with the Pi-Chromium decision in [[desktop-agent-lifecycle]]). `printReport()`/`updatePrintHead()` in `static/lens.html`; guards with an alert if no comparison has been run yet. **Fix (same day):** the first cut hid `#siteHeader`, but `site_header.js` *replaces* that placeholder with `<header class="site-topbar">` and appends the banner/fab to `<body>`, so they still printed. Print rule now hides `.site-topbar`, `#annBanner`/`.ann-banner`, `#annRestore`/`.ann-restore`, `#feedbackFab`, `#demoFeedbackModal` (plus the controls/chips) — the PDF keeps only the report header + the active view + totals. **Title dedupe (same day):** print also showed the on-screen page `.doc-header` ("Member Hours / Section-filtered roster…") on top of the report header → two titles. Print now hides the page-level title via `.page > .doc-header` (scoped so it doesn't also hide `#memPrintHead`'s inner `.doc-header`), and the report title was renamed to **👥 Member Hours Report**. **Orientation:** print `@page` set to **A4 landscape** (fits the weekly/monthly grids). → `static/lens.html`.

2026-05-31 — [lens] **Weekly grid now shows an off-days ratio + bar per weekday to flag regular days off.** Each weekday cell adds a red proportional bar and an `X/Y` label (X = off/leave days, Y = times that weekday occurs in the range). When a member is off on *every* occurrence (`4/4`) the bar goes solid red (`.full`) and the cell is highlighted (`.alloff`, light-red bg) — clearly identifying a recurring day off; partial off (`2/4`) draws a half bar. Computed in `renderGrid()` by also tracking `worked` count alongside `off`. → `static/lens.html`.

2026-05-31 — [lens] **Added a ✕ Clear button next to Compare.** Clears all selected members + the loaded result and resets to the empty prompt (also clears the search box / hides the dropdown). New `clearAll()` in `static/lens.html`. → `static/lens.html`.

2026-05-31 — [lens] **Added a third view — 🗓 Monthly grid.** Rows = selected members, one column per calendar day across the selected range, each cell = that day's worked hours (`休` for a leave/off day), with a per-member Total column. Wide ranges scroll horizontally and the member-name column is sticky. Built from the same `/api/members/compare` payload via a new `renderMonth()` + `_dateRange()`/`_fmtISO()` helpers (local-date formatting, not `toISOString`, to avoid a JST off-by-one). → `static/lens.html`.

2026-05-31 — [lens] **Search dropdown is now checkbox multi-select.** Each match in the member search dropdown carries a checkbox; ticking adds the member, unticking removes — and the dropdown stays open so several can be picked from one search (previously a click added one member and closed the list). Kept open by `preventDefault` on the dropdown's `mousedown` (so the input doesn't blur) + a `refreshDropdown()` re-render after each toggle to sync the checked state with the chips. New `toggleMember()` helper. → `static/lens.html`.

2026-05-31 — [lens] **Added a Daily strip ↔ Weekly grid view toggle on the Lens page.** New segmented switch (`📊 Daily strip` / `📅 Weekly grid`) next to Compare. The existing per-member day-strip (Y = days, X = hours) is unchanged and default; the new **Weekly grid** renders rows = selected members, columns = Mon–Sun, each cell = worked hours summed for that weekday across the range plus a `休 N` leave/off-day count, with a per-member week Total column. Both views reuse the same `/api/members/compare` payload (`members[].days[]`), so no backend change. Implemented in `static/lens.html` (`state.view`, `renderGrid()`, `.mem-grid`/`.mem-viewtoggle` CSS). → `static/lens.html`.

2026-05-31 — [lens] **Removed the section pills from the Lens page — search now spans both sections.** Dropped the `Both sections / 製造１課 / 製造２課` toggle (`#memSection`) and its pill click-handler from `static/lens.html`; `state.section` is fixed to `"all"`, so the type-ahead search covers the whole roster and the dropdown still shows each member's section. Multi-select (chips) and the date-range/Compare flow are unchanged. → `static/lens.html`.

2026-05-31 — [nav/lens] **New top-nav tab "Lens" — Member Hours moved off the Gantt page into its own page.** The 👥 Member Hours comparison view (section-filtered roster + multi-member day-strip + totals, backed by `/api/members/{list,compare}`) used to be a sub-tab inside `gantt.html`. It now lives on a dedicated page so the Gantt page is just the chart again.
  - **New page** `static/lens.html` served at `/lens` (new route in `main.py`, `no-store` like the other page routes). The page auto-runs `MemberHours.ensureLoaded()` on DOMContentLoaded (no tab toggle needed). Carries its own copy of the axis helpers (`WIN_START/WIN_HOURS/parseH/toAxis/toPct/fmtHHMM`) and the `.mem-*` CSS, plus the shared site header.
  - **`static/site_header.js`**: added the **Lens** nav link after Intake (`Status · Reports · Setup · Intake · Lens`), an `activeKey()` case (`/lens` → `lens`), and — important — added `lens` to the BASE-path regex so nav links + health pill resolve correctly under the `/attendance/*` mount. **Cache-bust:** bumped the `site_header.js?v=` query string on every page that loads it (`dashboard / console / reports / summary / gantt / management` → `?v=20260531-lens`) — without this the browsers kept the cached 4-tab header and the Lens tab never appeared.
  - **`static/gantt.html`**: removed the Member Hours pieces — the `#memWrap` markup, the 👥 Member Hours tab button, the `MemberHours` IIFE, the `.mem-*` CSS, the `$('memWrap')` reset in `showSection()`, and the now-dead `#memTab`/`#memWrap` mobile-hide selectors. Backup: `static/gantt.html.bak.20260531`.
  - **Auth/routing:** `/api/members/` is already in `GBL_PUBLIC_PREFIXES`, so the Lens data loads on the genbafms hosts; the `/lens` page itself sits behind the same login wall as the other browser pages. Verified loopback: `/lens` 200 (title + `#memWrap` + auto-init present), `/api/members/list` 200, `/gantt` still 200.
  - → `static/lens.html` (NEW), `main.py` (`/lens` route), `static/site_header.js`, `static/gantt.html`.

2026-05-31 — [docs] **Synced stale docs to the live code (no behavior change).** Audit found three doc↔code mismatches; code is correct, docs were fixed to match:
  - **Data Cleanup cap was documented as 5, code enforces 31.** `_CLEANUP_MAX_DATES = 31` (`main.py:8068`) is the real, working limit (intentional "full month per request" window). Fixed `README.md` v3.5 bullet ("hard 5-date cap" → "hard 31-date cap") and the two stale in-code comments (`main.py` ~8057 and the `cleanup_delete` docstring ~8367) to reference `_CLEANUP_MAX_DATES` instead of a hard-coded "5" so they can't drift again. Historical CHANGELOG entries (2026-05-01) left as-is — they record the cap as it was then.
  - **`PARTNER_API.md` was missing the live `/v1/attendance` endpoint** (defined `partner_api.py:384`, already advertised in `/v1/datasets`). Added it to the §3 test-links table and §4 reference with its real response shape (per-employee in/out + worked hours + per-section/grand totals; `section=` filter; `limit`/`offset`).
  - **`PARTNER_API.md` examples showed a flat response; every endpoint is actually enveloped** as `{ok, data, meta}` via `partner_api._envelope`. Added a "Response envelope" note (data-vs-meta, pagination keys, ETag/304) and corrected the `/ping` example from `{"ok":true,"partner":…,"server_time":…}` to the real `{"ok":true,"data":{"pong":true,…},"meta":{"partner":…,…}}`.
  - → `README.md`, `PARTNER_API.md`, `main.py` (comments only).

2026-05-28 — [auth/desktop-agent] **`POST /api/daily-packs/save-excel-batch` reachable with `X-API-Key` on `link.genbafms.com`.** Bug report from agent dev (2026-05-27): the watcher's three-step Excel chain `xlsx/upload → auto-extract-excel → save-excel-batch` was 401-ing on the third call only — even though all three carry the same `X-API-Key`. Root cause: the 2026-05-24 host-auth allowlist intentionally kept `save-excel-batch` behind the `gbl_session` wall on GenbaLink hosts (the desktop agent didn't chain into it yet at the time). The agent now does, so the wall was blocking writes for ~5 h until `attendance-doctor.timer` swept them in over loopback. PDF flow was unaffected because `/api/v1/pdf/auto-upload?save=true` is one call inside the `/api/v1/*` self-protected family.
  - **`GBL_PUBLIC_PREFIXES`** (main.py ~774): added `/api/daily-packs/save-excel-batch` with comment that the handler now enforces session-or-X-API-Key inline (so the wall isn't the only gate — the route remains write-protected on the GenbaLink hosts).
  - **`save_daily_pack_excel`** (main.py ~7734): added `request: Request` to signature and a top-of-handler guard — on wall hosts (`_gbl_host_requires_auth(request)`), accept either a logged-in user (`_gbl_request_user`) or a valid `X-API-Key` (`_resolve_key_label`); else 401 `authentication required`. Non-wall hosts (legacy `rnd.asiakawaii.com`, 127.0.0.1 doctor loopback) preserve their existing pass-through behavior — same model as `auto-extract-excel`, which the doctor and browser already rely on.
  - **Verified** on `Host: link.genbafms.com` after `systemctl restart attendance.service`:
    - **No auth → 401** `authentication required` (regression preserved).
    - **Valid `X-API-Key` (APP) + empty payload → 400** `products[] is empty` (auth passed, handler reached — THE FIX).
    - **Bogus `X-API-Key` → 401** (key resolution still strict).
    - **`POST /api/daily-packs/auto-extract-excel?date=2026-04-16` w/ APP key → 200** (regression preserved — picked `夜勤用日報２６．０４．１６.xlsm`).
  - **Impact:** Excel ingestion is now seconds-fresh instead of "eventually consistent within 5 h" via the doctor sweep. No agent-side change required; `save-batch-ok` should start appearing in the watcher's activity log immediately after each upload.
  - Backup: `main.py.bak.20260528_save_excel_batch`.
  - → `main.py` (two surgical edits — public-prefix entry + handler-level auth guard).
